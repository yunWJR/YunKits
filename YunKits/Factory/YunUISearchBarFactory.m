...
+ (UISearchBar *)createBarWithTarget:(id)target {
    ...
    return [self createBarByFrame:CGRectMake(0.0f, 2.0f, screenWide - 15 * 4 - 26 * 2, 26.0f) bgImgName:nil phText:@"搜索" target:target];
}

+ (UISearchBar *)createBarByPHText:(NSString *_Nullable)phText
                            target:(id _Nullable)target {
    CGFloat screenWide = [UIScreen mainScreen].bounds.size.width;
    return [self createBarByFrame:CGRectMake(0.0f, 2.0f, screenWide - 15 * 4 - 26 * 2, 26.0f) bgImgName:nil phText:phText target:target];
}

+ (UISearchBar *)createBarByFrame:(CGRect)frame
                           target:(id _Nullable)target {
    return [self createBarByFrame:frame bgImgName:nil phText:@"搜索" target:target];
}
...
+ (UIView *)createViewByFrame:(CGRect)frame
                       phText:(NSString *_Nullable)phText
                       target:(id _Nullable)target {
    UISearchBar *searchBar = [self createBarByFrame:frame bgImgName:nil phText:phText target:target];
    searchBar.tag = 99;
