...
+ (NSString *)mobilePhoneSimpleRgx {
    return @"^[0-9]{11}$";
}

/// 验证手机号
+ (NSString *)mobilePhoneFullRgx {
...
+ (NSString *)numAndLetterRgxInMin:(NSInteger)min max:(NSInteger)max {
    return FORMAT(@"^[A-Za-z0-9]{%@,%@}$", [YunValueHelper intStr:min], [YunValueHelper intStr:max]);
}

/// 由数字和26个英文字母组成的字符串
+ (NSString *)numAndLetterRgx {
    return @"^[A-Za-z0-9]+$";
}
...
+ (NSString *)numRgxInMin:(NSInteger)min max:(NSInteger)max {
    return FORMAT(@"^[0-9]{%@,%@}$", [YunValueHelper intStr:min], [YunValueHelper intStr:max]);
}
